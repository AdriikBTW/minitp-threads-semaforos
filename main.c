...
void descenderATierra(){
        printf("Al haber ganado las batallas, la nave nodriza desciende a la tierra \n");
}
void abrirCompuertas(){
        printf("Se abren las compuertas de la nave nodriza \n");
}
void abandonarTierra(){
        printf("Luego de abrirse las compuertas, un polen extraño comienza a aniquilar a los extraterrestres. El comandante ordena el retiro del planeta \n");
...
int main(void){
        ...
        //Creo los hilos
        for(int i=0;i<15;i++){
                pthread_create(&presidentes_threads[i],NULL,presidentes,NULL);
        }
        for(int i=0;i<33;i++){
                pthread_create(&naves_destructoras_threads[i],NULL,naves_destructoras,NULL);
        }
        for(int i=0;i<33;i++){
